...
Public Function fLicenceState() As Integer
    ...
    If fFExists(PzGSettingsFile) Then ' does the tool's own settings.ini exist?
        slicence = fGetINISetting("Software\PzCPUGauge", "licence", PzGSettingsFile)
...
Public Sub readPrefsPosition()
   ...
   On Error GoTo readPrefsPosition_Error
    ...
    If PzGDpiAwareness = "1" Then
        PzGFormHighDpiXPosTwips = fGetINISetting("Software\PzCPUGauge", "formHighDpiXPosTwips", PzGSettingsFile)
        PzGFormHighDpiYPosTwips = fGetINISetting("Software\PzCPUGauge", "formHighDpiYPosTwips", PzGSettingsFile)
...
'        End If
    Else
        PzGFormLowDpiXPosTwips = fGetINISetting("Software\PzCPUGauge", "formLowDpiXPosTwips", PzGSettingsFile)
        PzGFormLowDpiYPosTwips = fGetINISetting("Software\PzCPUGauge", "formLowDpiYPosTwips", PzGSettingsFile)
   ...
   On Error GoTo writePrefsPosition_Error
    ...
    If panzerPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If PzGDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzCPUGauge", "formHighDpiXPosTwips", PzGFormHighDpiXPosTwips, PzGSettingsFile
            sPutINISetting "Software\PzCPUGauge", "formHighDpiYPosTwips", PzGFormHighDpiYPosTwips, PzGSettingsFile
        Else
            PzGFormLowDpiXPosTwips = Str$(panzerPrefs.Left)
            PzGFormLowDpiYPosTwips = Str$(panzerPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzCPUGauge", "formLowDpiXPosTwips", PzGFormLowDpiXPosTwips, PzGSettingsFile
            sPutINISetting "Software\PzCPUGauge", "formLowDpiYPosTwips", PzGFormLowDpiYPosTwips, PzGSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(panzerPrefs.hwnd, "open", thisCommand, "Panzer CPU Gauge.exe prefs", "", 1)
    Else
